...
for graph_param in graph_params_list:
    for hem in hemispheres_list:
        for n_sl_points in n_sl_points_list:
            ...
            cmd = "qsub -cwd -q all.q -b y -V -e /netapp/vol1_psy/basepsy/FS60/%s/log_files/loop08_%s_%d_%d_%d_errors.txt -o /netapp/vol1_psy/basepsy/FS60/%s/log_files/loop08_%s_%d_%d_%d_output.txt python %s/08_singlescale_pointstat.py %s %s %s %d %d %d" % (experiment, hem, graph_param, n_sl_points, n_permuts, experiment, hem, graph_param, n_sl_points, n_permuts, coderoot_dir, experiment, hem, graph_type, graph_param, n_sl_points, n_permuts)
